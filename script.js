    ...
    try {
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                if (colorId !== 0) {
                    appState.colorTotals[colorId] = (appState.colorTotals[colorId] || 0) + 1;
        ...
        if (saved) {
            ...
            arr.forEach(p => {
                ...
                appState.colorPainted[colorId] = (appState.colorPainted[colorId] || 0) + 1;
...
function useMagicWand(startX, startY) {
    ...
    while (stack.length > 0) {
        ...
        appState.paintedPixels.add(pos);
        appState.colorPainted[colorToFill] = (appState.colorPainted[colorToFill] || 0) + 1;
        appState.paintedArray[y * appState.currentLevel.width + x] = 1;
...
function updateProgress() {
    ...
    for (const colorId in appState.colorTotals) {
        ...
        if (btn) {
            ...
            const colorPercent = colorTotal > 0 ? Math.floor((colorPainted / colorTotal) * 100) : 100;
